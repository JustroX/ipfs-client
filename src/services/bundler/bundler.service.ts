...
export class BundlerService {
  ...
  async bundle(type: 'file' | 'folder', source: string, passphrase: string) {
    // Create bundle directory
    const output_name = 'bundle.zip';
    ...
    await Archiver.zipFolder(bundle_root, 'content', path);
    await fs.rm(bundle_root, { recursive: true });

    return path;
  }

  async unbundle(source: string, passphrase: string) {
    // Create unbundle directory
    // const tmp_root = tmpdir();
    const output_path = await mkdir();
    const tmp_root = await mkdir();
